Page({
  ...
  update_rawData(){
    ...
    for(var i=index1;i<=index2;i++){
      list=this.mergeAndIntersectArrays(list,this.getclass_info(time[i]))
    }
    this.setData({
      rawData:list
    })
  ...
  // 数组的并集,对返回的每个节次的数据进行交集处理
  mergeAndIntersectArrays(arr1, arr2) {
    function intersectJsLists(jsList1, jsList2) {
      const idSet = new Set(jsList1.map(item => item.id));
      const intersectedJsList = jsList2.filter(item => idSet.has(item.id));
  ...
  // 获取空教室数据，返回数组
  getclass_info(selection){
    const qzapi=require("../../../../API/qzapi")
    qzapi.getClassroomInfo(selection).then(res=>{
      return res
    })
  },
